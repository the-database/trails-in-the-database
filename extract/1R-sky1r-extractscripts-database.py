...
def _decode_dialogue(args):
    ...
    # Some calls use a variable like `var0` or `var1` (party-slot lookup
    # result) as the speaker. We can't resolve at parse time, so we keep
    # the dialogue with chr_id=None and resolve via voice prefix later.
...
def build_chr_alias(parsed_files, jp_tname, tvoice):
    ...
    Voice files are recorded one-per-VA, so the filename prefix (`v013_*`
    for Richard, `v001_*` for Estelle, etc.) reliably identifies the
    speaker. We use this two ways:
      - `chr_alias[unknown_chr_id] -> known_chr_id`: for scenario chr_ids
        like 61810 that are absent from t_name.
      - `voice_prefix_to_known[prefix] -> known_chr_id`: fallback when
        chr_id can't be parsed at all (variable like `var0` in source).
    """
    from collections import Counter
    prefix_known = {}      # voice_prefix -> Counter(known_chr_id -> count)
    unk_prefixes = {}      # unknown_chr_id -> Counter(voice_prefix -> count)
...
def main():
    ...
    # Pre-pass: parse JP files and build chr_id alias map. JP suffices since
    # voice IDs are language-independent.
    ...
    print(f'  voice prefixes mapped to known speakers: {len(voice_prefix_to_known)}', file=sys.stderr)

    def _resolve_via_voice(voice_id, side_tname, side_field):
        """Last-ditch: if chr_id was a runtime variable, use voice prefix
        to identify the speaker (voice files are recorded per-VA)."""
        if voice_id is None:
            return ''
        fn = tvoice.get(voice_id) or ''
        if len(fn) < 4:
            return ''
        prefix = fn[:4]
        known_cid = voice_prefix_to_known.get(prefix)
    ...
    def portrait(chr_id, voice_id=None):
        if chr_id in ANONYMOUS_CHR_IDS:
            return None, None
        if chr_id is None:
            # Variable chr_id; resolve via voice prefix
            if voice_id is None:
                return None, None
            fn = tvoice.get(voice_id) or ''
            if len(fn) < 4:
