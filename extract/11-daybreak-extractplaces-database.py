"""Extract Trails through Daybreak (game 11) place names into daybreak-places.sql.

Companion to 11-daybreak-extractscripts-database.py. Structure follows db2's
places extractor (`t_place.utf8text` for display name, same `id`-based pairing).

Output schema matches the `file` table:
  delete from file where game_id = 11;
  insert into file (game_id, fname, place_name_eng, place_name_jpn) values
    ('11', '<map>', '<en name>', '<jp name>');
...
DAYBREAK_ROOT = os.path.join(os.path.expanduser('~'), 'Documents', 'programming', 'KuroTools', 'daybreak1_extract')
EN_TPLACE = os.path.join(DAYBREAK_ROOT, 'en_json', 't_place.json')
JP_TPLACE = os.path.join(DAYBREAK_ROOT, 'jp_json', 't_place.json')
GAME_ID = 11
OUTPUT = 'daybreak-places.sql'
