...
vita_jpn_path = './sen4-scripts/'
# pc_eng_path = './sen4-scripts/'
jpn_encode = 'utf8'
...
def get_talk_npcs():
    ...
    for fname in os.listdir(vita_jpn_path + 'scena/'):
        with open(os.path.join(vita_jpn_path, 'scena/', fname), 'rb') as f_jpn:
...
def get_calling_funcs(all_bytes_by_func, func_names, func_index, depth=0):

    # if depth > 10:
    #     return []

    calling_funcs = [i for i in range(len(func_names)) if
                     re.search(b'\x02[\x0b\x00]' + func_names[func_index] + b'\x00', all_bytes_by_func[i]) and i != func_index]
    for calling_func_index in calling_funcs:
        calling_funcs += get_calling_funcs(all_bytes_by_func, func_names, calling_func_index, depth + 1)
...
def get_voices_jpn():
    ...
    with open('sen4-text/t_voice.tbl', 'rb') as f:
...
def get_items_jpn():
    return get_items('./sen4-text/t_item.tbl', jpn_encode)
...
def get_names_jpn():
    return get_names('./sen4-text/t_name.tbl', jpn_encode)
...
def parse_str_with_voice(fs, is_jpn=True):
    global items_jpn, voices_jpn
    result = []
    voice_files = []
...
def get_func_positions(fs, is_scena=True):
    junk = fs.read(20)
    num_funcs = struct.unpack('<I', fs.read(4))[0]
    junk = fs.read(8)

    # fname = parse_str(fs)
    fname_junk = fs.read(8)
    # if is_scena:
    #     junk = fs.read(2)
    ...
    for i in range(len(lines)):
        if i < len(voice_files):
            new_lines.append(
                ...
                f'data-audio="talk/9/{voice_files[i]}.opus">{lines[i]}</a>'
            )
        else:
...
def do_everything2(jpn_subpath, eng_subpath, fname):
    global total_jpn, names_jpn_by_code, names_jpn_by_code

    if fname.endswith('.dat'):
        with open(os.path.join(vita_jpn_path, jpn_subpath, fname), 'rb') as f_jpn:
            ...
            f_jpn.seek(0)
            jpn_func_positions, jpn_func_names = get_func_positions(f_jpn, not fname.startswith('tk_'))
            f_jpn.seek(0)
            ...
            print("%04d" % len(all_stripped_jpn), fname)
...
for fname in os.listdir(vita_jpn_path + 'scena/'):
    do_everything2('scena/', 'scena/dat_en/', fname)
for fname in os.listdir(vita_jpn_path + 'talk/'):
    do_everything2('talk/', 'talk/dat_en/', fname)

# do_everything2('scena/', 'scena/dat_en', 'm5000.dat')
# do_everything2('scena/', 'scena/dat_en', 'm0100.dat')
# do_everything2('talk/', 'talk/dat_en', 'tk_zessica.dat')
# do_everything2('talk/', 'talk/dat_en', 'tk_vivi.dat')
# do_everything2('scena/', 'scena/dat_en', 'c0210.dat')
# do_everything2('talk/', 'talk/dat_en', 'tk_wayne.dat')

print('TOTAL JPN = %d' % (total_jpn))

# print('exit early')
# exit()

with open(r'sen4.sql', 'w', encoding='utf-8') as sqlfile:
    sqlfile.write('delete from script where game_id = 9;\n')
