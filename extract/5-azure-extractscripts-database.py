"""Extract Trails to Azure (game 5) dialogue into azure.sql.

Mirrors the game-4 (Zero) extractor — Azure uses the same decompiled-Python
scena format from EDDecompiler and the same dialogue ops. See
4-zero-extractscripts-database.py for the detailed format notes.
"""
...
EN_SCENA_ROOT = os.path.join(AZURE_ROOT, 'en', 'scena')
JP_TNAME = os.path.join(AZURE_ROOT, 'jp', 'text', 't_name._dt.json')
EN_TNAME = os.path.join(AZURE_ROOT, 'en', 'text', 't_name._dt.json')
GAME_ID = 5
OUTPUT = 'azure.sql'
...
def _normalize_voice_id(digits):
    ...
    source #1999990020V -> file v999990020.opus for Azure (10-digit codes),
    #10100762V -> v0100762.opus for Zero (8-digit codes). Short legacy
    codes (4-digit, 12 occurrences in Azure) are left untouched."""
